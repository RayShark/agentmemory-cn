...
import { ESSENTIAL_TOOLS, getAllTools } from "./mcp/tools-registry.js";
...
function renderHelpText(): string {
  const toolCountHelp = `(default: all = ${ALL_TOOLS_COUNT} tools; core = ${CORE_TOOLS_COUNT} essentials)`;
  return cliT("help.text", { pinnedVersion: IIPINNED_VERSION })
    .replace(
      /\(default: all = \d+ tools; core = \d+ essentials\)/,
      toolCountHelp,
    )
    .replace(
      /（默认：all = \d+ 个工具；core = \d+ 个核心工具）/,
      `（默认：all = ${ALL_TOOLS_COUNT} 个工具；core = ${CORE_TOOLS_COUNT} 个核心工具）`,
    );
}
...
if (toolsIdx !== -1 && args[toolsIdx + 1]) {
  ...
  if (toolsMode !== "all" && toolsMode !== "core") {
    p.log.warn(
      cliT("tools.unknownValue", { value: toolsMode }),
    );
  }
...
async function ensureIiiConsole(): Promise<IiiConsoleState> {
  ...
  if (!shBin || !curlBin) {
    p.log.warn(cliTFor(locale, "console.missingCurl", { command: iiiConsoleInstallHint() }));
    return state;
  }
  ...
  if (!ok) {
    p.log.warn(cliTFor(locale, "console.failed", { command: iiiConsoleInstallHint() }));
    return state;
  }
...
async function startEngine(): Promise<boolean> {
  ...
  if (iiiBin && configPath) {
    if (iiiBin !== pathIii) {
      p.log.info(
        cliTFor(locale, "engine.foundIii", {
          path: iiiBin,
          versionText: ` (v${IIPINNED_VERSION})`,
        }),
      );
  ...
  if (dockerOptIn && dockerBin && composeFile) {
    ...
    const detected = iiiBinVersion(pathIii!);
    p.log.info(
      cliTFor(locale, "engine.versionMismatch", {
        detected: detected ?? "unknown",
        agentVersion: VERSION,
        pinnedVersion: IIPINNED_VERSION,
        downloadHint: agentmemoryBinDir(),
      }),
    );
  } else if (!interactive) {
    choice = "install";
...
async function startServerForDemo(locale: Locale): Promise<() => Promise<void>> {
  ...
  if (startedEngine) {
    ...
    if (!ok) {
      p.log.error(cliTFor(locale, "engine.startFailed"));
      p.note(
        installInstructions(locale).join("\n"),
        cliTFor(locale, "engine.setupRequiredTitle"),
      );
      process.exit(1);
    }
    if (!(await waitForEngine(15000))) {
      p.log.error(cliTFor(locale, "engine.notReady"));
      process.exit(1);
    }
  }
  ...
  await import("./index.js");
  if (!(await waitForAgentmemoryReady(15000))) {
    p.log.error(cliTFor(locale, "demo.notReachable", { port: getRestPort() }));
    process.exit(1);
  }
...
async function runDemo() {
  ...
  if (serve) {
    teardown = await startServerForDemo(locale);
...
async function runDemoBody(base: string, locale: Locale) {
