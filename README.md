...
<p align="center">
  <img src="assets/banner.png" alt="agentmemory - 面向 AI 编码代理的持久化记忆" width="720" />
</p>

<p align="center">
  <strong>
    你的编程代理会记住所有上下文。无需反复解释。
    基于 <a href="https://github.com/iii-hq/iii">iii engine</a> 构建。
  </strong><br/>
  面向 Claude Code、GitHub Copilot CLI、Cursor、Gemini CLI、Codex CLI、Hermes、OpenClaw、pi、OpenCode 以及任何 MCP 客户端的持久记忆。
</p>
...
<p align="center">
  ...
  <a href="#powered-by-iii">iii 驱动</a> &bull;
...
如果你正在使用编程代理，最快的路径是把下面这一条指令交给它；它会端到端安装、接线并验证 agentmemory。

> 获取并执行这里的说明: https://raw.githubusercontent.com/RayShark/agentmemory-cn/v1.0.0/INSTALL_FOR_AGENTS.md

Windows 上建议走 WSL2。原生 Windows 的 engine 设置目前需要手动完成（大约 10 到 20 分钟），并且 `agentmemory connect` 暂不支持原生 Windows。步骤见下方 [Windows notes](#windows)。

```bash
npm install -g @agentmemory/agentmemory          # 安装一次，让 `agentmemory` 进入 PATH
# 如果 macOS/Linux 的系统级 Node 安装遇到 EACCES，改用：
# sudo npm install -g @agentmemory/agentmemory
agentmemory                                      # 在 :3111 启动记忆服务器
agentmemory demo                                 # 写入示例会话并验证召回
agentmemory demo --serve                         # 单命令：启动服务器、运行 demo、清理退出（无需第二个终端）
agentmemory connect claude-code                  # 把 MCP 接入你的代理（也支持 copilot-cli、codex、cursor、gemini-cli 等）
npx skills add RayShark/agentmemory-cn -y        # 安装 15 skills（8 个可调用技能、7 个参考技能），让代理知道何时使用这些工具
```
...
注意：`npx` 会按版本缓存。如果直接运行 `npx @agentmemory/agentmemory` 时启动了旧版本，可以用 `npx -y @agentmemory/agentmemory@latest` 强制拉取最新版，或清理一次缓存：macOS/Linux 上删除 `~/.npm/_npx`，Windows 上删除 `%LOCALAPPDATA%\npm-cache\_npx`。从 v0.9.16 起，首次 npx 运行会提示你顺手全局安装，之后就能直接使用 `agentmemory` 命令。

完整选项见下方 [快速开始](#quick-start)。不同代理的接线方式见 [代理支持](#works-with-every-agent)。
...
codex plugin marketplace add RayShark/agentmemory-cn
...
copilot plugin install RayShark/agentmemory-cn:plugin
```
...
```bash
npx skills add RayShark/agentmemory-cn -y          # auto-detects the calling agent
npx skills add RayShark/agentmemory-cn -y -a warp  # explicit agent
npx skills add RayShark/agentmemory-cn -y -a '*'   # install to every installed agent
```
...
- `npx skills add RayShark/agentmemory-cn` installs the skills so the agent knows when to call them.
...
| **GitHub Copilot CLI (full plugin)** | Copilot plugin install | `copilot plugin install RayShark/agentmemory-cn:plugin` for the plugin from the GitHub subdir. |
...
```bash
git clone https://github.com/RayShark/agentmemory-cn.git && cd agentmemory-cn
