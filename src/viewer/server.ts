...
export function resolveViewerHost(): string {
  return resolveViewerListenHost();
}
...
export function startViewerServer(
  ...
  server.on("listening", () => {
    ...
    if (inboundSecret !== null) {
      ...
      console.log(
        `[agentmemory] Viewer: http://${host}:${actualPort} (bound to ${host}; inbound Bearer required; allowed Host headers: ${allowedHosts})`,
      );
      return;
    }
    if (actualPort === requestedPort) {
      console.log(`[agentmemory] Viewer: http://${host}:${actualPort}`);
    } else {
      console.log(
        `[agentmemory] Viewer started on http://${host}:${actualPort} (fallback from ${requestedPort})`,
      );
    }
  });
