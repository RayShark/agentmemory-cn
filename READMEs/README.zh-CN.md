...
- 15 个 skills:`/recall`、`/remember`、`/session-history`、`/forget`
...
53 个工具、6 个资源、3 个提示词、15 个 skills — 任何代理可用的最全面 MCP 记忆工具包。
...
端口 `3111` 上的 128 个端点。REST API 默认绑定 `127.0.0.1`。当 `AGENTMEMORY_SECRET` 已设置时,受保护端点需要 `Authorization: Bearer <secret>`,网状同步端点要求两端都设置 `AGENTMEMORY_SECRET`。
